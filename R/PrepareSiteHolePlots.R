PrepareSiteHolePlots <- function(site_level_analysis,
                                 hole_data,
                                 dir_out){
  ...
  hole_metrics <- qs::qread(hole_data) |>
  ...
  fig.out <- file.path(dir_out,"site_level_plots","site-level-hole-volume.png") 
  
  
  .SiteLevelPlot(raw_data = hole_metrics,
  ...
  fig.out <- file.path(dir_out,"site_level_plots","site-level-hole-number.png") 
