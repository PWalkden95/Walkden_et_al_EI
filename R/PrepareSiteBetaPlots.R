PrepareSiteBetaPlots <- function(site_level_analysis, beta_data, dir_out) {
  ...
  purrr::map(realms, .f = function(x) {
    fig.out <- file.path(
      dir_out,
      "site_level_plots",
      glue::glue("site-level-similarity-{x}.png")
    )
  ...
  purrr::map(realms, .f = function(x) {
    fig.out <- file.path(
      dir_out,
      "site_level_plots",
      glue::glue("site-level-centorid-{x}.png")
    )
