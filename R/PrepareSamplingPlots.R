PrepareSamplingPlots <- function(predicts_path,
                                 dir_out){
  ...
  land_use_plot_data <-
    ...
    site_info |> dplyr::group_by(Predominant_habitat, Realm) |>
    ...
    dplyr::mutate(Predominant_habitat = factor(Predominant_habitat, levels = land_uses)) |>
  ...
  limits <- max(land_use_plot_data[,c(2:ncol(land_use_plot_data))], na.rm = TRUE) + 20
  ...
  purrr::map(realms, function(x) {
    ...
    # use category
    sample_plot <-
      ggplot2::ggplot(data = realm_land_use_data,
      ...
      fig.out <- file.path(dir_out,"sampling_plots",glue::glue("LU-sample-plot-{x}.png"))
    # save the plot
    
    ggplot2::ggsave(sample_plot,
                        filename = fig.out,
                        width = 250,
                        height = 200,
                        units = "mm",
  ...
  diet_data <- predicts |>
    dplyr::mutate(Trophic_Niche = ifelse(
      Trophic_Niche %in%
        c("Aq.p", "Hb.T", "Hb.A", "Vt"),
      "Other",
      Trophic_Niche
  ...
  diet_table_data <- diet_data  |>
    ...
    dplyr::distinct(Realm,Birdlife_Name,Predominant_habitat,.keep_all = TRUE) |>
...
purrr::map(realms, function(x) {
  ...
  # dietary guild
  diet_plot <-
    ggplot2::ggplot(data = diet_plot_data[, c("Trophic_Niche", x)], ggplot2::aes(
    ...
    ggplot2::geom_text(
      ...
      size = 15,
      fontface = "bold",angle = 90, hjust = 0
    ) +
  ...
  fig.out <- file.path(dir_out,"sampling_plots",glue::glue("diet-sample-plot-{x}.png"))
  # save the plot
  ggplot2::ggsave(plot = diet_plot,
         filename = file.path(dir_out,"sampling_plots",glue::glue("diet-sample-plot-{x}.png")),
         width = 100,
         height = 200,
         units = "mm",
         dpi = 500
...
worldwide_diet_plot$Trophic_Niche <- factor(worldwide_diet_plot$Trophic_Niche, levels = c(
  "Om","In","Ne","Fr","Gr"
))

rownames(worldwide_diet_plot) <- worldwide_diet_plot$Trophic_Niche
worldwide_diet_plot <- worldwide_diet_plot[c(
  "Om","In","Ne","Fr","Gr"
),]
...
# dietary guild
plot <-
  ggplot2::ggplot(data = worldwide_diet_plot,
                  ggplot2::aes(x = c(0.3, 0.65, 1, 1.35, 1.70), y = as.numeric(worldwide_diet_plot[,2]),
...
fig.out <- file.path(dir_out,"sampling_plots",glue::glue("diet-sample-plot-worldwide.png"))
# save the plot
