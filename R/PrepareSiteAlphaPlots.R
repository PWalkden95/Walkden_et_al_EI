PrepareSiteAlphaPlots <- function(site_level_analysis, alpha_data, dir_out) {
  ...
  purrr::map(realms, .f = function(x) {
    fig.out <- file.path(
      dir_out,
      "site_level_plots",
      glue::glue("site-level-alpha-{x}.png")
    )
